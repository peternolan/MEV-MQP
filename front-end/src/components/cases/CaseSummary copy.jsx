...
*/
  ...
  toggleThisWord = (event) => {
      (event.target.value === 'blue') ? console.log('b') : console.log('r')
  };
    ...
    return (
      <div key={this.state.caseName} className={this.props.classes.summaryContent} >
        ...
        <Collapse isOpened={this.state.keywordsExposed}>
          <div className={this.props.classes.keywordContainer} key="highlighted_words">
            {(this.state.highlightedWordsData.length === 0) ? 
             <Typography key='nowords' type='body1' style={{padding: 5, paddingLeft: 15}}>There are no annotated reports in this case for us to build keywords from; try annotating one of the reports.</Typography>
            : this.state.highlightedWordsData.map((word) => {
              return(
                <div value='blue' id={word.name} key={word.name} className={this.props.classes.keywordCapsule} onClick={this.toggleThisWord}>
