  ...
  drawChart = (reports) => {
    ...
    if (document.getElementById('legend-' + this.props.caseID)) {
      ...
      for (var z = 0 ; z < formatted_data.counts[formatted_data.fields.indexOf(label)].length; z++ ) {
        legendCont +=
            `<div className= ${this.props.classes.legendEntry} style  = 'color: #fff; font-size: .75em; padding: 2px; margin-right: ${'1%'}; background-color : ${"#"+this.getFillColor(z, formatted_data.counts[formatted_data.fields.indexOf(label)].length)}' >${ formatted_data.counts[formatted_data.fields.indexOf(label)][z].label}</div>`
      }
